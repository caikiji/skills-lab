---
name: developing-go
description: 涉及 Go 代码分析时**必须使用**——包括理解代码、调试、bug 定位、重构、代码审查、依赖分析、安全审计等，强制使用 gopls MCP/LSP、go doc、repomix，禁止 Grep/全量 Read 和并行 Read 多文件。
---
...
**禁止并行 Read 多文件**——需要读取多个文件时，必须用 repomix 打包，禁止并行调用多个 Read。
...
**积极使用 repomix**——需要读取多个文件时，必须用 repomix 打包（禁止并行 Read 的强制约束见上方）。
