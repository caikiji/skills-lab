...
3. **派活**：`herdr agent prompt worker-01 "<任务>。完整结论写入 .temp/reports/worker-01.md，回复只需该路径。" --wait --timeout 600000`；`--wait` 等首个 settled（idle/done/blocked）；agent 自有参数放 `--` 之后。
4. **验收**：以报告文件为准（输出可能落在 alternate screen 读不回）；`agent read --source recent-unwrapped` 看现场；`blocked` = 卡审批 UI，读屏后问用户，不替答。
5. **收尾**：`agent list` 确认无残留 working；退出 agent 归还面板（注入 `/exit`，前缀 `MSYS_NO_PATHCONV=1`），面板可复用。
