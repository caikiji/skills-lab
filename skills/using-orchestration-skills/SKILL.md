...
This skill is a router, not a replacement for the downstream skill. Its job is to decide whether to use `deepresearch`, `context-research-orchestrator`, or `semantic-batch-refactor-orchestrator`, and in what order.
...
- If the output must be reusable by downstream agents, plans, or later orchestration steps, prefer `context-research-orchestrator`.
- If the task is a large semantic code change that needs frozen rules and conflict-aware execution, prefer `semantic-batch-refactor-orchestrator`.
...
digraph orchestration_router {
    ...
    "Invoke context-research-\norchestrator" [shape=box];
    "Invoke semantic-batch-\nrefactor-orchestrator" [shape=box];
    "Invoke CRO first,\nthen SBRO" [shape=box];
    ...
    "Reusable context for\ndownstream agents needed?" -> "Invoke context-research-\norchestrator" [label="yes"];
    ...
    "Can rules be frozen\nsafely now?" -> "Invoke semantic-batch-\nrefactor-orchestrator" [label="yes"];
    "Can rules be frozen\nsafely now?" -> "Invoke CRO first,\nthen SBRO" [label="no"];
}
```
...
| The next step needs source-backed findings packaged for later agents or workflows | `context-research-orchestrator` |
| The task is a broad semantic refactor or migration with risky parallel execution | `semantic-batch-refactor-orchestrator` |
| The task is a semantic batch refactor, but the rules cannot yet be frozen safely | `context-research-orchestrator` first, then `semantic-batch-refactor-orchestrator` |
...
### Use `context-research-orchestrator` when
...
- a semantic refactor may be coming next, and research must feed SBRO

`context-research-orchestrator` produces a `Research Report` plus a `Context Pack` for downstream use.

### Use `semantic-batch-refactor-orchestrator` when
...
`semantic-batch-refactor-orchestrator` is for execution orchestration after the rules are clarified enough to freeze.

## CRO Before SBRO

Run `context-research-orchestrator` before `semantic-batch-refactor-orchestrator` when any of these are true:
...
If a fresh `Context Pack` already exists and is still usable, SBRO may consume it directly instead of re-running CRO.
...
| "This is obviously a migration, I can go straight into execution." | Confirm whether SBRO is the right skill and whether CRO is required first. |
...
- a replacement for the workflow inside `deepresearch`, `context-research-orchestrator`, or `semantic-batch-refactor-orchestrator`
...
2. If the choice is `CRO -> SBRO`, complete CRO first and carry its findings forward with their labels intact.
