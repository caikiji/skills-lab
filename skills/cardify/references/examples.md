...
◆ 卡 2/5 ｜ 概念卡 · 入口
一句话：所有支付请求统一进一个入口落单（pay/service.go:32）
...
◆ 卡 4/5 ｜ 流程卡 · 回调
一句话：第三方回调统一进队列串行消费（pay/callback.go:30）
...
| 锚点是纯符号名 | 错误: 卡2要点1：锚点须含 文件:行号 或目录路径 | 改 `（HeroDrawId）` 为 `（public/luban_bin/HeroDrawCardConfigBin.go:20）` |
